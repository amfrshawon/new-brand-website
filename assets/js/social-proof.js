...
(function () {
  ...
  function timeAgoLabel(isSpanish) {
    const minutes = 1 + Math.floor(Math.random() * 58);
    if (isSpanish) return minutes === 1 ? "hace 1 min" : `hace ${minutes} min`;
    return minutes === 1 ? "1 min ago" : `${minutes} min ago`;
  }

  function buildCard(sale, isSpanish) {
    const el = document.createElement("div");
    el.className = "sp-card sp-enter";
    const title = isSpanish ? `${sale.title} vendido` : `${sale.title} sold`;
    el.innerHTML = `
      <div class="sp-card-ico">${CAR_ICON}</div>
      <div class="sp-card-body">
        <p class="sp-card-title">${title}</p>
        <p class="sp-card-sub">${sale.city} · ${timeAgoLabel(isSpanish)}</p>
      </div>`;
    return el;
  }
  ...
  function init() {
    ...
    const initialCards = shuffled.slice(0, 4).map((s) => buildCard(s, isSpanish));
    ...
    setInterval(() => {
      ...
      const card = buildCard(nextSale, isSpanish);
